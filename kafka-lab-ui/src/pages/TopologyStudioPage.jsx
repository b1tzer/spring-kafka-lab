...
import { LAB_REALTIME_EVENT_TYPE, TOPOLOGY_EVENT_TYPE } from '../constants/labDomain';
...
const toSnapshot = (payload) => {
  ...
  if (payload.brokers && payload.topics && (payload.producers || payload.consumers)) {
    return payload;
  }
...
const TopologyStudioInner = () => {
  ...
  const rafRef = useRef();
  const particlesRef = useRef(Array.from({ length: MAX_PARTICLES }, () => ({ active: false })));
  ...
  useEffect(() => {
    const graph = buildFlowGraph({ snapshot: topologySnapshot, producerRate, consumerRate });
    setNodes(graph.nodes);
    setEdges(graph.edges);
  }, [topologySnapshot, producerRate, consumerRate, setNodes, setEdges]);

  const bumpPartitionCounter = useCallback((partitionId) => {
  ...
  const shakeConsumer = useCallback((consumerId) => {
    setNodes((current) => current.map((node) => {
  ...
  useEffect(() => {
    ...
    const spawnFromEvent = () => {
      ...
      if (staged.stage === 'SEND_ONLY' || staged.stage === 'CHAINED') {
        ...
        if (from && to) {
          particlesRef.current[poolIndex] = {
            ...
            next: staged.stage === 'CHAINED'
              ? {
                fromId: staged.fromPartitionGlobalId || staged.toPartitionGlobalId,
      ...
      if (staged.stage === 'RECV_ONLY' || staged.stage === 'CHAINED') {
        const from = anchorFor(nodes, staged.fromPartitionGlobalId || staged.toPartitionGlobalId, 'right');
    ...
    const frame = () => {
      ...
      particlesRef.current.forEach((particle, index) => {
        ...
        const point = pointOnCubic(particle.p0, particle.p1, particle.p2, particle.p3, Math.min(1, particle.t));
        ...
        particle.trail.forEach((trailPoint, trailIndex) => {
          ...
          ctx.beginPath();
          ctx.fillStyle = `${particle.color}${Math.floor(alpha * 140).toString(16).padStart(2, '0')}`;
  ...
  return (
    <div className="topology-studio-page">
      <div className="topology-studio-head">
        <Space>
          <Typography.Title level={4} style={{ margin: 0, color: '#dce8ff' }}>Topology Studio</Typography.Title>
          {runningEnvironment && <Tag color="blue">{runningEnvironment.name}</Tag>}
        </Space>
      ...
      <div className="topology-canvas-wrap">
        ...
        >
          ...
          <MiniMap zoomable pannable style={{ background: '#0f172a', border: '1px solid #334155' }} />
          <Controls />
        </ReactFlow>
        <canvas
          ref={canvasRef}
