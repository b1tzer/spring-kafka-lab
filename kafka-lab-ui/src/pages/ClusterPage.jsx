...
const ClusterPage = () => {
          ...
          <Table
            ...
            expandable={{
              expandedRowRender: (partitionRow) => {
                const rows = (partitionRow.recentMessages || []).map((message, index) => ({
                  key: `${partitionRow.key}-${message.offset}-${index}`,
                  ...message
                }));

                if (rows.length === 0) {
                  return <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No messages" />;
                }
                  ...
                  <div className="memory-board">
                    {rows.map((row) => (
                      <div className="memory-row" key={row.key}>
                        <div className="memory-cell memory-time">{row.timestamp || '-'}</div>
                        <div className="memory-cell memory-offset">offset {row.offset}</div>
                        <div className="memory-cell memory-key">key {row.key}</div>
                        <div className="memory-cell memory-value" title={String(row.value)}>{String(row.value)}</div>
                        <div className="memory-cell memory-producer">{row.producerId || '-'}</div>
                      </div>
                    ))}
                  </div>
                );
            ...
            columns={[
              ...
              {
                title: 'Latest 10',
                render: (_, row) => <Tag color="blue">{(row.recentMessages || []).length}</Tag>,
                width: 120
              }
            ]}
          />
  ...
  return (
    <Space direction="vertical" style={{ width: '100%' }}>
      ...
      {!ready && <Alert type="warning" showIcon message="请先在 Environments 页面创建并启动一个 Kafka 环境，再查看集群信息" />}
        ...
        <Spin spinning={loading}>
          <div className="topology-graph">
            <div className="graph-lane graph-left">
              <Typography.Text strong>Producers ({activeProducers.length})</Typography.Text>
              {producerNodes.length === 0 ? (
                <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No active producer" />
              ) : (
                producerNodes.map((producer) => (
                  <div className="node-link-row" key={producer.key}>
                    <div className={`node-card producer-node ${producerPulse ? 'node-pulse' : ''}`}>
                      <UploadOutlined />
                      <div className="node-main">{producer.label}</div>
                      <div className="node-sub">{producer.topic || '-'}</div>
                    </div>
                    <div className={`link-line ${producerPulse ? 'link-producer-animate' : ''}`} />
                  </div>
                ))
              )}
            </div>
                ...
                <div className="broker-grid">
                  {brokers.map((broker) => (
                    <div className="broker-chip" key={broker.id}>
                      ...
                      <Badge status={broker.status === 'UP' ? 'success' : 'error'} text={broker.status} />
                      <Tag>{broker.activeController ? 'controller-active' : 'follower'}</Tag>
                    </div>
                  ))}
                </div>
                <div className="cluster-meta">
                  <Tag icon={<ApartmentOutlined />}>{`topics: ${topics.length}`}</Tag>
            ...
            <div className="graph-lane graph-right">
              <Typography.Text strong>Consumers ({activeConsumers.length})</Typography.Text>
              {consumerNodes.length === 0 ? (
                <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No active consumer" />
              ) : (
                consumerNodes.map((consumer) => (
                  <div className="node-link-row" key={consumer.key}>
                    <div className={`link-line ${consumerPulse ? 'link-consumer-animate' : ''}`} />
                    <div className={`node-card consumer-node ${consumerPulse ? 'node-pulse' : ''}`}>
                      <DownloadOutlined />
