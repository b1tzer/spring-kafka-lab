...
const ScenariosPage = () => {
  ...
  return (
    <Space direction="vertical" style={{ width: '100%' }}>
      ...
      {!ready && <Alert type="warning" showIcon message="请先在 Environments 页面创建并启动一个 Kafka 环境，再运行实验场景" />}
      {ready && runningEnvironment && <Alert type="success" showIcon message={`当前环境：${runningEnvironment.name}`} />}
      <Card>
        <Space wrap>
          {scenarioButtons.map((item) => (
            <Button key={item.name} type="primary" disabled={!ready} onClick={() => onRun(item.name)}>
              {item.label}
            </Button>
          ))}
        </Space>
