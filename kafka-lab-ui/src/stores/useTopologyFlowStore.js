...
const useTopologyFlowStore = create((set, get) => ({
  ...
  markConsumerRecv: (event) => set((state) => {
    const second = nowSecond();
    const bucket = state.consumerRate[event.toClientId] || { second, count: 0 };
    const nextBucket = bucket.second === second
      ? { second, count: bucket.count + 1 }
      : { second, count: 1 };
  ...
  consumeMatchedEvent: () => {
    ...
    if (recv) {
      ...
      set((current) => ({
        ...
        sendByMsgId: send
          ? Object.fromEntries(Object.entries(current.sendByMsgId).filter(([msgId]) => msgId !== recv.id))
          : current.sendByMsgId
      }));
      if (send) {
        return {
  ...
  triggerRipple: (partitionId) => set((state) => ({
    activeRipples: {
      ...state.activeRipples,
  ...
  triggerConsumerShake: (consumerId) => set((state) => ({
    consumerShake: {
      ...state.consumerShake,
      [consumerId]: Date.now()
    }
