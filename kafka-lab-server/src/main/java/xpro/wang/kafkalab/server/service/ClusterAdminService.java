        ...
        try (AdminClient adminClient = adminClient()) {
            ...
            return Map.of(
                ...
                try (AdminClient adminClient = adminClient()) {
                    ...
                    List<Map<String, Object>> brokers = nodes.stream().map(node -> Map.of(
                        "id", node.id(),
                        "host", node.host(),
                        "port", node.port(),
                    ...
                    List<Map<String, Object>> topicItems = topicAdminService.listTopics().stream()
                        .sorted()
                        .map(topicName -> {
                        try {
                            Map<String, Object> detail = topicAdminService.describeTopic(topicName);
                            @SuppressWarnings("unchecked")
                            List<Map<String, Object>> partitions = (List<Map<String, Object>>) detail.getOrDefault("partitions", List.of());

                            List<Map<String, Object>> enhancedPartitions = partitions.stream().map(partition -> {
                            int partitionId = (int) partition.get("partition");
                            List<Map<String, Object>> recentMessages = producerLabService.recentMessages(topicName, partitionId, 10);
                            return Map.of(
                                "partition", partitionId,
                                "leader", partition.getOrDefault("leader", "N/A"),
                                "replicas", partition.getOrDefault("replicas", List.of()),
                    ...
                    return Map.of(
                        ...
                        "brokers", brokers,
                        "producer", Map.of(
                            "activeCount", activeProducers.size(),
                            "active", activeProducers
                        ),
                        "consumer", Map.of(
                            "activeCount", activeConsumers.size(),
                            "subscriptions", activeConsumers
                        ),
                        "topics", topicItems
                    );
                }
