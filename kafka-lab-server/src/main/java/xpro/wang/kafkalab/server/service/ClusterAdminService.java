...
public class ClusterAdminService {
    ...
    public ClusterAdminService(RuntimeKafkaConnectionService runtimeKafkaConnectionService,
            TopicAdminService topicAdminService, ProducerLabService producerLabService,
    ...
    public ClusterSummary clusterSummary() throws Exception {
        try (AdminClient adminClient = adminClient()) {
            ...
            String controller = describeCluster.controller().get() == null
                    ? "N/A"
                    : describeCluster.controller().get().idString();

            List<String> brokers = nodes.stream().map(node -> node.idString() + "@" + node.host() + ":" + node.port())
                    .toList();

            return new ClusterSummary(describeCluster.clusterId().get(), controller, brokers, brokers.size());
        }
    }
    ...
    public List<String> brokers() throws Exception {
        try (AdminClient adminClient = adminClient()) {
            return adminClient.describeCluster().nodes().get().stream()
                    .map(node -> node.idString() + "@" + node.host() + ":" + node.port()).toList();
        }
    }
    ...
    public Map<String, Object> dashboard() throws Exception {
        ...
        try (AdminClient adminClient = adminClient()) {
            ...
            return Map.of("brokerCount", brokers().size(), "topicCount", topics.size(), "consumerGroupCount",
                    groups.size());
        }
    }
    ...
    public Map<String, Object> topology() throws Exception {
        try (AdminClient adminClient = adminClient()) {
            ...
            List<Map<String, Object>> brokers = nodes.stream()
                    .map(node -> Map.of("id", node.id(), "host", node.host(), "port", node.port(), "rack",
                            node.rack() == null ? "N/A" : node.rack(), "status", "UP", "roles",
                            List.of("broker", "controller-eligible"), "activeController",
                            controllerId != null && controllerId == node.id()))
                    .toList();

            List<Map<String, Object>> topicItems = topicAdminService.listTopics().stream().sorted().map(topicName -> {
                try {
                    Map<String, Object> detail = topicAdminService.describeTopic(topicName);
                    @SuppressWarnings("unchecked")
                    List<Map<String, Object>> partitions = (List<Map<String, Object>>) detail.getOrDefault("partitions",
                            List.of());

                    List<Map<String, Object>> enhancedPartitions = partitions.stream().map(partition -> {
                        int partitionId = (int) partition.get("partition");
                        List<Map<String, Object>> recentMessages = producerLabService.recentMessages(topicName,
                                partitionId, 10);
                        return Map.of("partition", partitionId, "leader", partition.getOrDefault("leader", "N/A"),
                                "replicas", partition.getOrDefault("replicas", List.of()), "recentMessages",
                                recentMessages);
                    }).toList();

                    return Map.of("topic", topicName, "partitionCount", detail.getOrDefault("partitionCount", 0),
                            "partitions", enhancedPartitions);
                } catch (Exception ex) {
                    return Map.of("topic", topicName, "partitionCount", 0, "partitions", List.of(), "error",
                            ex.getMessage());
                }
            }).toList();
            ...
            return Map.of("clusterId", describe.clusterId().get(), "kafkaMode", "KRaft", "brokerCount", brokers.size(),
                    "brokers", brokers, "producer",
                    Map.of("activeCount", activeProducers.size(), "active", activeProducers), "consumer",
                    Map.of("activeCount", activeConsumers.size(), "subscriptions", activeConsumers), "topics",
                    topicItems);
        }
    }
