...
public class EnvironmentController {
   ...
   */
  @PostMapping("/create")
  public ApiResponse<EnvironmentInstance> create(
      @Valid @RequestBody EnvironmentCreateRequest request) throws Exception {
    EnvironmentInstance created = environmentManagerService.create(request);
    labRealtimeWebSocketHandler.publish(
        LabRealtimeEventType.ENVIRONMENT_CHANGED,
        java.util.Map.of(
   ...
   */
  @GetMapping("/{id}/logs")
  public ApiResponse<EnvironmentActionResult> logs(
      @PathVariable String id, @RequestParam(defaultValue = "200") int tail) throws Exception {
