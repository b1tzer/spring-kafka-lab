...
public class EnvironmentController {
    ...
    public ApiResponse<EnvironmentInstance> create(@Valid @RequestBody EnvironmentCreateRequest request)
        ...
        labRealtimeWebSocketHandler.publish(LabRealtimeEventType.ENVIRONMENT_CHANGED, java.util.Map.of("action",
                LabRealtimeAction.CREATED.name(), "id", created.id(), "name", created.name()));
    ...
    public ApiResponse<EnvironmentActionResult> start(@PathVariable String id) throws Exception {
        ...
        labRealtimeWebSocketHandler.publish(LabRealtimeEventType.ENVIRONMENT_CHANGED,
                java.util.Map.of("action", LabRealtimeAction.STARTED.name(), "id", id, "success", result.success()));
    ...
    public ApiResponse<EnvironmentActionResult> stop(@PathVariable String id) throws Exception {
        ...
        labRealtimeWebSocketHandler.publish(LabRealtimeEventType.ENVIRONMENT_CHANGED,
                java.util.Map.of("action", LabRealtimeAction.STOPPED.name(), "id", id, "success", result.success()));
    ...
    public ApiResponse<EnvironmentActionResult> delete(@PathVariable String id) throws Exception {
        ...
        labRealtimeWebSocketHandler.publish(LabRealtimeEventType.ENVIRONMENT_CHANGED,
                java.util.Map.of("action", LabRealtimeAction.DELETED.name(), "id", id, "success", result.success()));
