...
public class ScenarioEngineService {
    ...
    public ScenarioEngineService(ProducerLabService producerLabService, ConsumerLabService consumerLabService) {
    ...
    public Map<String, Object> run(ScenarioRunRequest request) throws Exception {
        ...
        return switch (scenarioName) {
            ...
            case "broker-crash" ->
                Map.of("scenario", "broker-crash", "status", ScenarioStatus.MANUAL_STEP_REQUIRED.name(), "hint",
                        "Use docker compose stop kafka2 then observe leader election");
            case "exactly-once" -> runExactlyOnce(parameters);
            default ->
                Map.of("scenario", request.scenarioName(), "status", ScenarioStatus.NOT_SUPPORTED.name(), "supported",
                        new String[]{"rebalance", "consumer-lag", "poison-message", "broker-crash", "exactly-once"});
        };
    }
    ...
    private Map<String, Object> runRebalance(Map<String, Object> parameters) {
        ...
        return Map.of("scenario", "rebalance", "status", ScenarioStatus.TRIGGERED.name(), "groupId", groupId, "topic",
                topic, "hint", "Check consumer group assignment changes in Kafka UI");
    }
    ...
    private Map<String, Object> runLag(Map<String, Object> parameters) throws Exception {
        ...
        producerLabService.send(new ProducerSendRequest(topic, null, "lag-message", null, 0L, count, false));

        return Map.of("scenario", "consumer-lag", "status", ScenarioStatus.TRIGGERED.name(), "topic", topic,
                "messageCount", count, "hint", "Start slow consumer and observe lag metrics");
    }

    private Map<String, Object> runPoison(Map<String, Object> parameters) {
        return Map.of("scenario", "poison-message", "status", ScenarioStatus.MANUAL_STEP_REQUIRED.name(), "hint",
    ...
    private Map<String, Object> runExactlyOnce(Map<String, Object> parameters) throws Exception {
        ...
        producerLabService.send(new ProducerSendRequest(topic, "eos-key", "eos-message", null, 0L, 3, true));
        return Map.of("scenario", "exactly-once", "status", ScenarioStatus.TRIGGERED.name(), "topic", topic, "hint",
