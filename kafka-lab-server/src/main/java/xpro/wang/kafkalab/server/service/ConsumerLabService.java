...
public class ConsumerLabService {
  ...
  public Map<String, Object> registerConsumer(ConsumerRegisterRequest request) {
    ...
    log.info(
        "Managed consumer registered: clientId={}, groupId={}, topics={}, autoCommit={}",
        clientId,
        groupId,
        topics,
        managedConsumer.autoCommit());
  ...
  public Map<String, Object> stopConsumer(String groupId) {
    List<String> matched =
        managedConsumers.values().stream()
  ...
  public Map<String, Object> startByClientId(String clientId) {
    ...
    worker.start();
    log.info(
        "Managed consumer started: clientId={}, groupId={}, topics={}",
        managed.clientId(),
        managed.groupId(),
        managed.topics());

    ManagedConsumer updated =
        new ManagedConsumer(
            managed.clientId(),
            managed.groupId(),
            managed.hostIp(),
  ...
  public Map<String, Object> stopByClientId(String clientId) {
    ...
    log.info(
        "Managed consumer stopped: clientId={}, groupId={}", managed.clientId(), managed.groupId());
  ...
  public Map<String, Object> updateConsumerTopics(String clientId, List<String> rawTopics) {
    ...
    log.info(
        "Managed consumer topics updated: clientId={}, topics={}, restarted={}",
        clientId,
        topics,
        wasRunning);
  ...
  public Map<String, Object> deleteByClientId(String clientId) {
    ...
    log.info(
        "Managed consumer deleted: clientId={}, groupId={}", removed.clientId(), removed.groupId());
  ...
  public Map<String, Object> listGroups() throws Exception {
    ...
    try (AdminClient adminClient = adminClient()) {
      ...
      Map<String, ConsumerGroupDescription> descriptions =
          groups.isEmpty() ? Map.of() : adminClient.describeConsumerGroups(groups).all().get();
          ...
          groups.stream()
              .map(
                  group -> {
                    ...
                    item.put(
                        "state",
                        description == null ? "UNKNOWN" : String.valueOf(description.state()));
                    item.put("members", description == null ? 0 : description.members().size());
                    item.put(
                        "managedByLab",
                        managedConsumers.values().stream()
                            .anyMatch(c -> c.groupId().equals(group)));
                    return item;
                  })
              .toList();
  ...
  private KafkaConsumer<String, String> consumerOf(ManagedConsumer managed) {
    ...
    props.put(
        ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, runtimeKafkaConnectionService.bootstrapServers());
  ...
  private final class ConsumerWorker {
    ...
    private void runLoop() {
      try (KafkaConsumer<String, String> consumer = consumerOf(managed)) {
        ...
        log.info(
            "Consumer worker subscribed: clientId={}, topics={}",
            managed.clientId(),
            managed.topics());
        ...
        while (running) {
          ...
          for (ConsumerRecord<String, String> record : records) {
            ...
            payload.put(
                "fromPartitionGlobalId",
                "b"
                    + record.leaderEpoch().orElse(0)
                    + "_t"
                    + record.topic()
                    + "_p"
                    + record.partition());
        ...
        running = false;
        log.error(
            "Consumer worker failed: clientId={}, groupId={}",
            managed.clientId(),
            managed.groupId(),
            ex);
      }
