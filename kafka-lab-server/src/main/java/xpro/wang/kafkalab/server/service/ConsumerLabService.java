...
public class ConsumerLabService {
    ...
    public Map<String, Object> registerConsumer(ConsumerRegisterRequest request) {
        ...
        ManagedConsumer managedConsumer = new ManagedConsumer(clientId, groupId,
                request.hostIp() == null || request.hostIp().isBlank() ? "127.0.0.1" : request.hostIp(), topics,
                request.autoCommit() == null || request.autoCommit(), ConsumerRuntimeStatus.STOPPED.name(),
                Instant.now().toString(), null);
        managedConsumers.put(clientId, managedConsumer);
        log.info("Managed consumer registered: clientId={}, groupId={}, topics={}, autoCommit={}", clientId, groupId,
                topics, managedConsumer.autoCommit());
        return toMap(managedConsumer);
    }

    public List<Map<String, Object>> listManagedConsumers() {
        return managedConsumers.values().stream().sorted((a, b) -> a.clientId().compareToIgnoreCase(b.clientId()))
    ...
    public Map<String, Object> startConsumer(ConsumerStartRequest request) {
        ...
        registerConsumer(new ConsumerRegisterRequest(request.groupId(), clientId, "127.0.0.1", List.of(request.topic()),
                request.autoCommit()));
        return startByClientId(clientId);
    }

    public Map<String, Object> stopConsumer(String groupId) {
        List<String> matched = managedConsumers.values().stream().filter(consumer -> consumer.groupId().equals(groupId))
                .map(ManagedConsumer::clientId).toList();
        if (matched.isEmpty()) {
    ...
    public Map<String, Object> startByClientId(String clientId) {
        ...
        worker.start();
        log.info("Managed consumer started: clientId={}, groupId={}, topics={}", managed.clientId(), managed.groupId(),
                managed.topics());

        ManagedConsumer updated = new ManagedConsumer(managed.clientId(), managed.groupId(), managed.hostIp(),
                managed.topics(), managed.autoCommit(), ConsumerRuntimeStatus.RUNNING.name(), managed.createdAt(),
    ...
    public Map<String, Object> stopByClientId(String clientId) {
        ...
        ManagedConsumer updated = new ManagedConsumer(managed.clientId(), managed.groupId(), managed.hostIp(),
                managed.topics(), managed.autoCommit(), ConsumerRuntimeStatus.STOPPED.name(), managed.createdAt(),
                managed.startedAt());
        managedConsumers.put(clientId, updated);
    ...
    public Map<String, Object> updateConsumerTopics(String clientId, List<String> rawTopics) {
        ...
        ManagedConsumer updated = new ManagedConsumer(managed.clientId(), managed.groupId(), managed.hostIp(), topics,
                managed.autoCommit(),
                wasRunning ? ConsumerRuntimeStatus.RUNNING.name() : ConsumerRuntimeStatus.STOPPED.name(),
                managed.createdAt(), wasRunning ? Instant.now().toString() : managed.startedAt());
        managedConsumers.put(clientId, updated);
    ...
    public Map<String, Object> deleteByClientId(String clientId) {
        ...
        return Map.of("clientId", clientId, "groupId", removed.groupId(), "status", "DELETED");
    }
    ...
    public Map<String, Object> listGroups() throws Exception {
        ...
        try (AdminClient adminClient = adminClient()) {
            ...
            return Map.of("groups", items, "localRegistry",
                    managedConsumers.values().stream().map(this::toMap).toList());
        }
    }

    public List<Map<String, Object>> localConsumers() {
        return managedConsumers.values().stream()
                .filter(item -> ConsumerRuntimeStatus.RUNNING.name().equals(item.status())).map(item -> {
    ...
    private AdminClient adminClient() {
        return AdminClient.create(
                Map.of(AdminClientConfig.BOOTSTRAP_SERVERS_CONFIG, runtimeKafkaConnectionService.bootstrapServers()));
    }
    ...
    private record ManagedConsumer(String clientId, String groupId, String hostIp, List<String> topics,
            boolean autoCommit, String status, String createdAt, String startedAt) {
    }
            ...
            try (KafkaConsumer<String, String> consumer = consumerOf(managed)) {
                ...
                while (running) {
                    ...
                    for (ConsumerRecord<String, String> record : records) {
                        String msgId = managed.clientId() + "-" + record.topic() + "-" + record.partition() + "-"
                                + record.offset();
                        ...
                        payload.put("fromPartitionGlobalId", "b" + record.leaderEpoch().orElse(0) + "_t"
                                + record.topic() + "_p" + record.partition());
                        ...
                        Map<String, Object> activity = labActivityLogService.append("info", "CONSUMED",
                                "Consumer consumed message", record.topic(),
                                Map.of("consumer", managed.clientId(), "groupId", managed.groupId(), "topic",
                                        record.topic(), "partition", record.partition(), "offset", record.offset(),
                                        "key", record.key() == null ? "" : record.key(), "value",
